...
final class Cron_Health {
    ...
    public static function status(): array {
        ...
        if ( defined( 'DISABLE_WP_CRON' ) && DISABLE_WP_CRON ) {
            return [
                ...
                'message'         => __( 'Server cron active (DISABLE_WP_CRON = true). Webhooks and jobs run on schedule.', 'autonode-pro'),
                'next_event'      => $next ? sprintf(
                    /* translators: %s: human-readable time difference, e.g. "5 minutes" */
                    __( '%s from now', 'autonode-pro'),
        ...
        if ( $last_ping === 0 ) {
            $msg = __( 'Cron has never fired. Either the site has no traffic or WP-Cron is disabled.', 'autonode-pro');
            $st  = 'stale';
        } elseif ( $age > self::STALE_SECONDS ) {
            $mins = (int) ceil( $age / 60 );
            $msg  = sprintf( 
                /* translators: %d: number of minutes */
                __( 'Last cron ping was %d minutes ago - WP-Cron may not be running. Add a real server cron job.', 'autonode-pro'), 
                $mins 
            );
            $st   = 'stale';
        } else {
            ...
            $msg  = sprintf( 
                /* translators: %d: number of minutes */
                __( 'Cron is healthy. Last ping %d minute(s) ago.', 'autonode-pro'), 
                $mins 
            );
            $st   = 'ok';
        }
    ...
    private static function list_hooks(): array {
        $hooks = [
            'autonode_prune_logs'        => __( 'Prune activity log (daily)', 'autonode-pro'),
            'autonode_prune_rate_limits' => __( 'Prune rate limit buckets (hourly)', 'autonode-pro'),
            'autonode_fire_webhook'      => __( 'Webhook delivery (single)', 'autonode-pro'),
            'autonode_retry_webhook'     => __( 'Webhook retry (single)', 'autonode-pro'),
            'autonode_cron_health_ping'  => __( 'Health ping (5 min)', 'autonode-pro'),
            'autonode_prune_brute_force' => __( 'Prune brute-force log (hourly)', 'autonode-pro'),
        ];
        $result = [];
