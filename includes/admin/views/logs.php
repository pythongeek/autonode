...
<div class="amp-header">
    <div class="amp-header-left">
        <h1 class="amp-page-title"><?php esc_html_e( 'Activity Log', 'autonode-pro'); ?></h1>
        <span class="amp-badge-pill"><?php
                /* translators: %d: number of log entries */
                printf( esc_html__( '%d entries', 'autonode-pro'), count( $autonode_logs ) );
            ?></span>
    </div>
    <div class="amp-header-right" style="display:flex;gap:10px">
        <a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=autonode_export_logs&format=csv&nonce=' . wp_create_nonce( 'autonode_admin' ) ) ); ?>" class="amp-btn amp-btn-ghost button"><?php esc_html_e( 'Export CSV', 'autonode-pro'); ?></a>
        <a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=autonode_export_logs&format=json&nonce=' . wp_create_nonce( 'autonode_admin' ) ) ); ?>" class="amp-btn amp-btn-ghost button"><?php esc_html_e( 'Export JSON', 'autonode-pro'); ?></a>
        <button id="amp-clear-logs-btn" class="amp-btn amp-btn-danger button"><?php esc_html_e( 'Clear All Logs', 'autonode-pro'); ?></button>
    </div>
</div>
<div class="amp-card" style="padding:14px 20px;margin-bottom:16px">
    <form method="get" style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
        <?php wp_nonce_field( 'autonode_logs_filter', '_an_nonce' ); ?>
        <input type="hidden" name="page" value="autonode-logs">
        <?php
        $autonode_status_filter = 0;
        ...
        <select name="status" class="amp-select" style="max-width:160px;padding:7px 10px;font-size:12px" onchange="this.form.submit()">
            <option value=""><?php esc_html_e( 'All Statuses', 'autonode-pro'); ?></option>
        ...
        <a href="<?php echo esc_url( admin_url( 'admin.php?page=autonode-logs' ) ); ?>" class="amp-btn amp-btn-ghost button" style="padding:6px 12px;font-size:12px"><?php esc_html_e( 'Clear', 'autonode-pro'); ?></a>
    </form>
</div>
<div class="amp-card" style="padding:0;overflow:hidden">
    <table class="amp-table amp-log-table widefat wp-list-table">
        <thead><tr>
            <th><?php esc_html_e( 'Time (UTC)', 'autonode-pro'); ?></th>
            <th><?php esc_html_e( 'Key', 'autonode-pro'); ?></th>
            <th><?php esc_html_e( 'Action', 'autonode-pro'); ?></th>
            <th><?php esc_html_e( 'Object', 'autonode-pro'); ?></th>
            <th><?php esc_html_e( 'Method', 'autonode-pro'); ?></th>
            <th><?php esc_html_e( 'Status', 'autonode-pro'); ?></th>
            <th><?php esc_html_e( 'Duration', 'autonode-pro'); ?></th>
            <th><?php esc_html_e( 'IP', 'autonode-pro'); ?></th>
        </tr></thead>
        <tbody>
        <?php if ( empty( $autonode_logs ) ) : ?>
            <tr><td colspan="8" class="amp-empty-cell"><?php esc_html_e( 'No log entries.', 'autonode-pro'); ?></td></tr>
        <?php else : ?>
            <?php
            foreach ( $autonode_logs as $autonode_log ) :
    ...
    <div style="display:flex;gap:8px;padding:14px 20px;border-top:1px solid var(--amp-border)">
        ...
        ?>
        <?php if ( $autonode_off > 0 ) : ?><a href="<?php echo esc_url( add_query_arg( 'offset', max( 0, $autonode_off - 100 ) ) ); ?>" class="amp-btn amp-btn-ghost" style="font-size:12px"><?php esc_html_e( 'Prev', 'autonode-pro'); ?></a><?php endif; ?>
        <?php if ( count( $autonode_logs ) === 100 ) : ?><a href="<?php echo esc_url( add_query_arg( 'offset', $autonode_off + 100 ) ); ?>" class="amp-btn amp-btn-ghost" style="font-size:12px"><?php esc_html_e( 'Next', 'autonode-pro'); ?></a><?php endif; ?>
    </div>
</div>
</div>
