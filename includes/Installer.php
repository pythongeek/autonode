...
final class Installer {
    ...
    public static function activate( bool $network_wide = false ): void {
        if ( is_multisite() && $network_wide ) {
            self::run_for_all_blogs( [ self::class, 'activate_blog' ] );
        } else {
            self::activate_blog();
        }
    }

    public static function activate_blog(): void {
        self::create_tables();
        ...
        add_option( 'autonode_settings', [
            ...
            'key_rotation_days'  => 0, /* Enterprise: Force expire keys */
            'require_https'      => false,
    ...
    public static function deactivate( bool $network_wide = false ): void {
        if ( is_multisite() && $network_wide ) {
            self::run_for_all_blogs( [ self::class, 'deactivate_blog' ] );
        } else {
            self::deactivate_blog();
        }
    }

    public static function deactivate_blog(): void {
    ...
    public static function uninstall(): void {
        ...
        if ( is_multisite() ) {
            self::run_for_all_blogs( [ self::class, 'uninstall_blog' ] );
        } else {
            self::uninstall_blog();
        }
    }

    public static function uninstall_blog(): void {
        global $wpdb;
        $tables = [
    ...
    private static function run_for_all_blogs( callable $callback ): void {
        global $wpdb;
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- multisite blog enumeration requires direct query
