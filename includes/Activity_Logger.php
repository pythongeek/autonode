...
final class Activity_Logger {
    ...
    public static function export( string $format = 'csv' ): void {
        global $wpdb;
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- export needs all rows, no caching needed
        $logs = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}autonode_log ORDER BY created_at DESC LIMIT 10000", ARRAY_A );
        if ( ! $logs ) return;

        $filename = 'autonode-logs-' . current_time( 'Y-m-d' );
        ...
        // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose -- php://output is not a real filesystem path
        fclose( $out );
        exit;
    }
}
