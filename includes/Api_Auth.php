...
final class Api_Auth {
    ...
    public static function authenticate( \WP_REST_Request $req ): array|\WP_Error {
        ...
        if ( ! empty( $settings['require_https'] ) && ! is_ssl() && ! ( defined( 'WP_DEBUG' ) && WP_DEBUG ) ) {
            return new \WP_Error( 'amp_ssl_required', __( 'HTTPS required.', 'autonode-pro'), [ 'status' => 403 ] );
        }
        ...
        if ( empty( $auth ) || ! str_starts_with( $auth, 'Bearer ' ) ) {
            return new \WP_Error( 'amp_no_auth', __( 'Missing Authorization header.', 'autonode-pro'), [ 'status' => 401 ] );
        }

        $raw = trim( substr( $auth, 7 ) );
        if ( ! str_starts_with( $raw, self::PREFIX ) ) {
            Brute_Force::record_failure( $ip );
            return new \WP_Error( 'amp_bad_token', __( 'Malformed token.', 'autonode-pro'), [ 'status' => 401 ] );
        }
        ...
        /* Key Rotation Check (Enterprise) */
        if ( ! empty( $settings['key_rotation_days'] ) ) {
            $days = (int) $settings['key_rotation_days'];
            $created = strtotime( $row['created_at'] );
            if ( ( time() - $created ) > ( $days * DAY_IN_SECONDS ) ) {
                return new \WP_Error( 'amp_key_expired', sprintf( 'API key expired due to rotation policy (%d days). Please rotate your key.', $days ), [ 'status' => 401 ] );
            }
        }
