...
final class Bulk_Controller extends Base_Controller {
    ...
    public static function handle( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        ...
        if ( ! $operation || empty( $items ) ) {
            return self::fail( $req, new \WP_Error( 'amp_invalid', __( 'operation and items required.', 'autonode-pro'), [ 'status' => 400 ] ), 'bulk' );
        }
        if ( count( $items ) > 50 ) {
            return self::fail( $req, new \WP_Error( 'amp_too_many', __( 'Max 50 items per request.', 'autonode-pro'), [ 'status' => 400 ] ), 'bulk' );
        }
        ...
        foreach ( $items as $item ) {
            ...
            try {
                $r = match ( $operation ) {
                    ...
                    default       => [ 'error' => sprintf( __( 'Unknown operation: %s', 'autonode-pro'), $operation ) ],
                };
    ...
    private static function delete( int $id ): array {
        ...
        return $r ? [ 'deleted' => true ] : [ 'error' => __( 'Delete failed', 'autonode-pro') ];
    }

    private static function update_seo( int $id, array $seo ): array {
        if ( ! $seo ) return [ 'error' => __( 'No SEO data provided', 'autonode-pro') ];
        return Rankmath_Handler::update( $id, $seo );
    }

    private static function update_meta( int $id, array $meta ): array {
        if ( ! $meta ) return [ 'error' => __( 'No meta data provided', 'autonode-pro') ];
