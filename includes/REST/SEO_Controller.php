...
final class SEO_Controller extends Base_Controller {
    ...
    public static function show( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        ...
        if ( ! get_post( $id ) ) return self::fail( $req, new \WP_Error( 'amp_not_found', __( 'Post not found.', 'autonode-pro'), [ 'status' => 404 ] ), 'get_seo' );
    ...
    public static function update( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        ...
        if ( ! get_post( $id ) ) return self::fail( $req, new \WP_Error( 'amp_not_found', __( 'Post not found.', 'autonode-pro'), [ 'status' => 404 ] ), 'update_seo' );
