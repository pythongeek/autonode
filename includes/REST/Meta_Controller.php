...
final class Meta_Controller extends Base_Controller {
    ...
    public static function delete_key( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        ...
        if ( ! get_post( $id ) ) {
            return self::fail( $req, new \WP_Error( 'amp_not_found', __( 'Post not found.', 'autonode-pro'), [ 'status' => 404 ] ), 'delete_meta' );
        }
        if ( self::is_blocked( $key ) ) {
            return self::fail( $req, new \WP_Error( 'amp_forbidden', __( 'Cannot delete protected meta key.', 'autonode-pro'), [ 'status' => 403 ] ), 'delete_meta' );
        }
