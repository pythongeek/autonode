...
final class Media_Controller extends Base_Controller {
    ...
    public static function show( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        ...
        if ( ! $a || $a->post_type !== 'attachment' ) {
            return self::fail( $req, new \WP_Error( 'amp_not_found', __( 'Media not found.', 'autonode-pro'), [ 'status' => 404 ] ), 'get_media' );
        }
    ...
    public static function upload( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        ...
        if ( empty( $body['file_base64'] ) || empty( $body['filename'] ) ) {
            return self::fail( $req, new \WP_Error( 'amp_invalid', __( 'file_base64 and filename are required.', 'autonode-pro'), [ 'status' => 400 ] ), 'upload_media' );
        }
        ...
        if ( strlen( $body['file_base64'] ) > ( $max_mb * 1024 * 1024 * 1.35 ) ) {
            /* translators: %d: file size limit in megabytes */
            return self::fail( $req, new \WP_Error( 'amp_too_large', sprintf( __( 'File exceeds limit of %dMB.', 'autonode-pro'), $max_mb ), [ 'status' => 413 ] ), 'upload_media' );
        }

        $decoded = base64_decode( $body['file_base64'], true ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions
        if ( ! $decoded ) {
            return self::fail( $req, new \WP_Error( 'amp_invalid', __( 'Invalid base64 data.', 'autonode-pro'), [ 'status' => 400 ] ), 'upload_media' );
        }
    ...
    public static function sideload( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        ...
        if ( ! $url || ! filter_var( $url, FILTER_VALIDATE_URL ) ) {
            return self::fail( $req, new \WP_Error( 'amp_invalid', __( 'A valid "url" field is required.', 'autonode-pro'), [ 'status' => 400 ] ), 'sideload_media' );
        }
        ...
        if ( ! is_wp_error( $head ) ) {
            ...
            if ( $cl > ( $max_mb * 1024 * 1024 ) ) {
                /* translators: %d: file size limit in megabytes */
                return self::fail( $req, new \WP_Error( 'amp_too_large', sprintf( __( 'Remote file exceeds limit of %dMB.', 'autonode-pro'), $max_mb ), [ 'status' => 413 ] ), 'sideload_media' );
            }
        }
        ...
        if ( ! $decoded ) {
            return self::fail( $req, new \WP_Error( 'amp_fetch_failed', __( 'Could not read downloaded file.', 'autonode-pro'), [ 'status' => 502 ] ), 'sideload_media' );
        }
    ...
    private static function save_upload( \WP_REST_Request $req, string $bytes, array $meta, string $action = 'upload_media' ): \WP_REST_Response|\WP_Error {
        ...
        /* Validate MIME type against WordPress allowed types */
        $mime_type = $meta['mime_type'] ?? wp_check_filetype( $filename )['type'] ?? 'application/octet-stream';
        $allowed   = get_allowed_mime_types();
        $ext       = strtolower( pathinfo( $filename, PATHINFO_EXTENSION ) );
        $valid_ext = ! empty( $allowed ) && in_array( $mime_type, $allowed, true );

        if ( ! $valid_ext ) {
            /* translators: %s: MIME type */
            return self::fail( $req, new \WP_Error( 'amp_bad_filetype', sprintf( __( 'File type %s is not allowed.', 'autonode-pro'), $mime_type ), [ 'status' => 415 ] ), $action );
        }
    ...
    public static function destroy( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        $id = (int) $req->get_param( 'id' );
        if ( ! get_post( $id ) ) {
            return self::fail( $req, new \WP_Error( 'amp_not_found', __( 'Media not found.', 'autonode-pro'), [ 'status' => 404 ] ), 'delete_media' );
        }
