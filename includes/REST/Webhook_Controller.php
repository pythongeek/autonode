...
final class Webhook_Controller extends Base_Controller {
    ...
    public static function create( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        ...
        if ( ! self::is_url_safe( $url ) ) {
            return self::fail( $req, new \WP_Error( 'amp_invalid', 'URL must be a public HTTPS endpoint.', [ 'status' => 400 ] ), 'create_webhook' );
        }
    ...
    public static function show( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        ...
        if ( ! $row ) return self::fail( $req, new \WP_Error( 'amp_not_found', __( 'Webhook not found.', 'autonode-pro'), [ 'status' => 404 ] ), 'get_webhook' );
    ...
    public static function update( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        ...
        if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}autonode_webhooks WHERE id=%d", $id ) ) ) { // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            return self::fail( $req, new \WP_Error( 'amp_not_found', __( 'Webhook not found.', 'autonode-pro'), [ 'status' => 404 ] ), 'update_webhook' );
        }
        $upd = [];
        if ( isset( $body['label'] )      ) $upd['label']      = sanitize_text_field( $body['label'] );
        if ( isset( $body['target_url'] ) ) {
            $upd_url = esc_url_raw( $body['target_url'] );
            if ( ! filter_var( $upd_url, FILTER_VALIDATE_URL ) ) {
                return self::fail( $req, new \WP_Error( 'amp_invalid', 'Valid target_url required.', [ 'status' => 400 ] ), 'update_webhook' );
            }
            if ( ! self::is_url_safe( $upd_url ) ) {
                return self::fail( $req, new \WP_Error( 'amp_invalid', 'URL must be a public HTTPS endpoint.', [ 'status' => 400 ] ), 'update_webhook' );
            }
            $upd['target_url'] = $upd_url;
        }
    ...
    /**
     * Block localhost, private, and link-local URLs.
     * Prevents SSRF via webhook delivery.
     */
    private static function is_url_safe( string $url ): bool {
        ...
        if ( $ip === $host ) {
            // Couldn't resolve — let it through (will fail at delivery time)
            return true;
        }
        ...
        return ! (
            ...
            || str_starts_with( $host, '172.16.' ) || str_starts_with( $host, '172.17.' ) || str_starts_with( $host, '172.18.' ) || str_starts_with( $host, '172.19.' ) || str_starts_with( $host, '172.20.' ) || str_starts_with( $host, '172.21.' ) || str_starts_with( $host, '172.22.' ) || str_starts_with( $host, '172.23.' ) || str_starts_with( $host, '172.24.' ) || str_starts_with( $host, '172.25.' ) || str_starts_with( $host, '172.26.' ) || str_starts_with( $host, '172.27.' ) || str_starts_with( $host, '172.28.' ) || str_starts_with( $host, '172.29.' ) || str_starts_with( $host, '172.30.' ) || str_starts_with( $host, '172.31.' )
            || str_starts_with( $host, '192.168.' )
            || str_starts_with( $host, '169.254.' ) // link-local
        );
    }
}
...
final class System_Controller extends Base_Controller {
    ...
    public static function ping( \WP_REST_Request $req ): \WP_REST_Response {
        $resp = new \WP_REST_Response( [ 'pong' => true, 'time' => current_time( 'c', true ) ], 200 );
        $resp->header( 'X-Request-ID', wp_generate_uuid4() );
        return $resp;
    }
