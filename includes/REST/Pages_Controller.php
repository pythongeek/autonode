...
final class Pages_Controller extends Base_Controller {
    ...
    public static function show( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        ...
        $post = get_post( $id );
        if ( ! $post || $post->post_type !== 'page' ) return self::fail( $req, new \WP_Error( 'amp_not_found', __( 'Page not found.', 'autonode-pro'), [ 'status' => 404 ] ), 'get_page' );
    ...
    public static function update( \WP_REST_Request $req ): \WP_REST_Response|\WP_Error {
        ...
        $body = $req->get_json_params() ?: [];
        $post = get_post( $id );
        if ( ! $post || $post->post_type !== 'page' ) return self::fail( $req, new \WP_Error( 'amp_not_found', __( 'Page not found.', 'autonode-pro'), [ 'status' => 404 ] ), 'update_page' );
